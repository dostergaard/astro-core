...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Min value: {:.2}", pixels.iter().fold(f32::INFINITY, |a, &b| a.min(b)));
    println!("Max value: {:.2}", pixels.iter().fold(f32::NEG_INFINITY, |a, &b| a.max(b)));
    ...
    println!("Background uniformity: {:.1}%", bg_metrics.uniformity * 100.0);
