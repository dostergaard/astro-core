            ...
            if parts.len() >= 2 {
                ...
                if let Some(location) = extract_attribute(&xml_content, "location") {
                    ...
                    if loc_parts.len() >= 3 && loc_parts[0] == "attachment" {
                        ...
                        // Read the pixel data
                        reader.seek(SeekFrom::Start(data_offset)).context("Failed to seek to image data")?;
                        
                        let mut data = vec![0u8; data_size];
                        reader.read_exact(&mut data).context("Failed to read image data")?;
...
fn read_pixel_data(data: &[u8], width: usize, height: usize) -> Result<Vec<f32>> {
    ...
    println!("Expected data size: {} bytes, actual: {} bytes", expected_size, data.len());
